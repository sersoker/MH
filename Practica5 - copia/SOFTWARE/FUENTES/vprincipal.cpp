...
// Clasificador knn
/////////////////////////////////////////////////////////////////////////////////
int knn(const vector<vector <double> > &v,vector<vector <double> > &vNormalizado,int ejemplo, vector<int> &mask,int numeroFilas,int numeroAtributos){
	vector<vector <double> > vConMascara;
	aplicarMascara(vNormalizado,vConMascara,mask,numeroFilas,numeroAtributos);
...
void llamadaam1(int numSimulacion){
	...
	calcularMinMax(mitad1,minimoColumnas,maximoColumnas,filasMitad1,numeroAtributos);
	normalizar(mitad1,vNormalizado,filasMitad1,numeroAtributos,minimoColumnas,maximoColumnas);

	vector<int> mask;
	vector<int> resultado;

	am1(mitad1,vNormalizado,mask,filasMitad1,numeroAtributos,resultado);
	...
	calcularMinMax(mitad2,minimoColumnas,maximoColumnas,mitad2.size(),numeroAtributos);
	normalizar(mitad2,vNormalizado2,mitad2.size(),numeroAtributos,minimoColumnas,maximoColumnas);
	for(int i=0;i<mitad2.size();i++)
		if(knn(mitad2,vNormalizado2,i,resultado,mitad2.size(),numeroAtributos)==mitad2[i][numeroAtributos-1])
			contadoraciertos++;
		else
			contadorfallos++;
	...
	cout << porcentaje;
	cout << 100*(((numeroAtributos-1)-(contadorCaracteristicas))/(numeroAtributos-1));

	cout << double( clock() - startTime ) / (double)CLOCKS_PER_SEC<<endl;
	
	//cout << "ACIERTOS: "<<contadoraciertos << "   FALLOS: "<<contadorfallos<<endl;
